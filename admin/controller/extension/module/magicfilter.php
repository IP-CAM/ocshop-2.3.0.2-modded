...
class ControllerExtensionModuleMagicfilter extends Controller {
    ...
    public function install() {
        ...
        $this->db->query("INSERT INTO " . DB_PREFIX . "setting SET store_id = '" . (int) 0 . "', `code` = '" . $this->db->escape('modulespacial') . "', `key` = '" . $this->db->escape('magicfilter_spacial') . "', `value` = '" . (int) 0 . "'");
...
//this function get recored from language file , saving recored  in setting file and setoutput in view file. 
    public function index() {
        ...
        $data['breadcrumbs'][] = array(
            ...
            'href' => $this->url->link('extension/module/magicfilter', 'token=' . $this->session->data['token'], true)
        );




        $data['action'] = $this->url->link('extension/module/magicfilter', 'token=' . $this->session->data['token'], true);
    ...
    public function test() {
        $magicfilter_library = new Moduleinfo($this->registry);
        $this->registry->set('Moduleinfo', $magicfilter_library);
        $result_cur = $magicfilter_library->test_module();
    }
