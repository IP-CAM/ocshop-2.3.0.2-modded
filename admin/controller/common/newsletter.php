...
class ControllerCommonNewsletter extends Controller {
	...
	protected function getList() {
		...
		foreach($result as $res)
		{
			$data['newsltr'][] = array(
				'id' => $res['id'],
				'email' => $res['email'],
				'delete' => $this->url->link('common/newsletter/delete', 'token=' . $this->session->data['token'] . '&id='.$res['id'], true)
			);
		}
		...
		$pagination->url = $this->url->link('common/newsletter', 'token=' . $this->session->data['token'] . '&page={page}', true);
	...
	public function delete() {
		...
		$this->response->redirect($this->url->link('common/newsletter', 'token=' . $this->session->data['token'], true));
	}
